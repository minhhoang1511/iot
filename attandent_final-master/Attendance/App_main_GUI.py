...
def FC_DangNhap():
    ...
    def btn_ChonCamera():
        ...
        def show_face():
            try:
                ...
                img = img.resize((620, 480), Image.Resampling.LANCZOS)
        ...
        def btn_themNguoiMoi():
                            ...
                            if (last_ret is not None) and (latest_frame is not None):
                                ...
                                if (len(faces) == 1):
                                    for (x, y, w, h) in faces:
                                        ...
                                        cv2.imwrite("anh_data_hinh/User." + id + '.' + str(sampleNum) + ".jpg",
                                                    gray[y:y + h, x:x + w])
                    ...
                    while (True):
                        if (last_ret is not None) and (latest_frame is not None):
                            ...
                            if (len(faces) == 1):
                                for (x, y, w, h) in faces:
                                    sampleNum = sampleNum + 1
                                    cv2.imwrite("data_test1/" + name + "." + id_test + '.' + str(sampleNum) + ".jpg",
                                                gray[y:y + h, x:x + w])
                                if (sampleNum > 99):
            ...
            lbl_ID_nguoiMoi = Label(tk_themNguoi, text="Mã số", bd=4, fg="white", font=("Times New Roman", 16),
                                    width=10, bg="green")
            ...
            lbl_Ten_nguoiMoi = Label(tk_themNguoi, text="Họ tên", bd=4, fg="white", font=("Times New Roman", 16),
                                     width=10, bg="green")
            ...
            btn_themNguoi = Button(tk_themNguoi, text="Thêm người", font=("Times New Roman", 14), fg="white",
                                   bg="green",
                                   width=15, height=1, command=btn_themNguoi)
            btn_themNguoi.place(x=100, y=90)
            btn_them_data_test = Button(tk_themNguoi, text="Thêm vào tập test", font=("Times New Roman", 14),
                                        fg="white", bg="green",
            ...
            try:
                while True:
                    ...
                    for (x, y, w, h) in faces:
                        ...
                        if conf > 70:
                            cv2.putText(img, str(conf) + "% : " + str(profile[1]), (x + 10, y - 20), fontface,
                                        fontscale, fontcolor, 2)
                            dem = 0
                            # print('gio','ddsng',Gio,start_dd_sang)
                            # print('Giờ: ', Gio)
                            # print('start_dd_sang: ', start_dd_sang)
                            # print('end_dd_sang: ', end_dd_sang)
                            # print('Loại Giờ: ', type(Gio))
                            # print('Loại start_dd_sang: ', type(start_dd_sang))
                            # print('Loại end_dd_sang: ', type(end_dd_sang))
                            ...
                            if dem < 1:
                                ...
                                for imagePath in imagePaths:
                                    ...
                                    if (c < 24 and (ID_ss == int(profile[0]))):
                                        ...
                                        cv2.imwrite("data_diemdanh_dung/" + str(profile[1]) + '.' + str(
                                            profile[0]) + '.' + str(image_dd) + ".jpg", img_cv)
                                    elif (c > 24 and ID_ss != int(profile[0])):
                                        print("Điểm danh sai rồi: ", profile[1])
                                        cv2.imwrite("data_diemdanh_sai/" + str(profile[1]) + '.' + str(
                                            profile[0]) + '.' + str(image_dd) + ".jpg", img_cv)
                            # else:
        ...
        def btn_dsdiemdanh():
            ...
            tk_ds.resizable(False, False)
            tk_ds.configure(bg="CornflowerBlue")
            lbl_title_ds = Label(tk_ds, text="DANH SÁCH ĐIỂM DANH NGAY " + str(x), font=("Times New Roman", 18),
                                 fg="green")
            ...
            def btn_thongke():
                ...
                lbl_title_ds_ch = Label(tk_tk, text="DANH SÁCH NHÂN VIÊN CHƯA ĐIỂM DANH", font=("Times New Roman", 18),
                                        fg="green")
        ...
        def btn_ddhomnay():
            ...
            lbl_title_dshn = Label(tk_hn, text="DANH SÁCH ĐIỂM DANH NGAY HÔM NAY", font=("Times New Roman", 18),
                                   fg="green")
        ...
        def btn_doiAdmin():
            ...
            lbl_new_username = Label(tk_doiPass, text="UserName new", bd=4, fg="white", font=("Times New Roman", 16),
                                     width=10, bg="green")
            ...
            lbl_pass_new = Label(tk_doiPass, text="PassWord new", bd=4, fg="white", font=("Times New Roman", 16),
                                 width=10, bg="green")
        ...
        def btn_ddhinhanh():
            ...
            img_nd = ImageTk.PhotoImage(Image.open("anhchupnhandang.jpg").resize((620, 480), Image.Resampling.LANCZOS))
            ...
            try:
                for (x, y, w, h) in faces:
                    ...
                    if (conf < 90):
                        dem = 0
                        if ((Gio > start_dd_sang and Gio < end_dd_sang) or (
                                Gio > start_dd_chieu and Gio < end_dd_chieu)):
                            for n in ids:
                                if (int(n) == int(profile[0])):
                                    dem = dem + 1
                            if (dem < 1):
                                ...
                                img_cv = cv2.cvtColor(gray[y:y + h, x:x + w], cv2.COLOR_GRAY2RGB)
                                cv2.imwrite(
                                    "data_hinh_diemdanh/anhchup" + '.' + str(profile[0]) + '.' + str(image_dd) + ".jpg",
                                    img_cv)
                                path2 = 'data_sosanh'
                                imagePaths = [os.path.join(path2, f) for f in os.listdir(path2)]
                                for imagePath in imagePaths:
                                    ID_ss = int(os.path.split(imagePath)[-1].split('.')[1])
                                    hashs = imagehash.average_hash(Image.open(
                                        "data_hinh_diemdanh/anhchup" + '.' + str(profile[0]) + '.' + str(
                                            image_dd) + ".jpg"))
                                    otherhash = imagehash.average_hash(Image.open(imagePath))
                                    c = hashs - otherhash
                                    if (c < 22 and (ID_ss == int(profile[0]))):
                                        sql_DB.insert_ttdiemdanh(profile[0], profile[1], Ngay, Gio)
                                        cv2.imwrite(
                                            "data_diemdanh_dung/" + str(profile[1]) + '.' + str(profile[0]) + '.' + str(
                                                image_dd) + ".jpg", img_cv)
        ...
        btn_danhsachNguoi = Button(tk_main, text="Danh sách nhân viên", font=("Times New Roman", 14), fg="white",
                                   bg="green",
        ...
        btn_ddhinhanh = Button(tk_main, text="Chụp hình điểm danh", font=("Times New Roman", 14), fg="white",
                               bg="green",
                               width=18, height=1, command=btn_ddhinhanh)
        btn_ddhinhanh.place(x=250, y=600)
        btn_diemdanhrealtime = Button(tk_main, text="Điểm danh real time", font=("Times New Roman", 14), fg="white",
                                      bg="green",
                                      width=18, height=1, command=btn_diemdanhrealtime)
        btn_diemdanhrealtime.place(x=10, y=600)
        btn_dsdiemdanh = Button(tk_main, text="Danh sách theo ngày", font=("Times New Roman", 14), fg="white",
                                bg="green",
    ...
    lbl_title = Label(tk_cam, text="Nhập path video or stream camera (0 or ' ' camera máy tính)",
                      font=("Times New Roman", 18), fg="green", bg="white")
    lbl_title.place(x=30, y=10)
    btn_ChonCamera = Button(tk_cam, text="Chọn thiết bị video/stream", font=("Times New Roman", 14), fg="white",
                            bg="green",
...
lbl_title = Label(root, text="ỨNG DỤNG ĐIỂM DANH VỚI CÔNG NGHỆ NHẬN DẠNG KHUÔN MẶT", font=("Times New Roman", 20),
                  fg="red")
